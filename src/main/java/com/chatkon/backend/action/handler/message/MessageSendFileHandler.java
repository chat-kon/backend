...
import com.chatkon.backend.model.dto.message.MessageSendFileRequestDto;
import com.chatkon.backend.model.dto.message.MessageSendFileResponseDto;
...
public class MessageSendFileHandler implements ActionHandler<MessageSendFileRequestDto> {
    ...
    public ActionType type() {
        return ActionType.SEND_FILE;
    }

    @Override
    public ActionResult handle(Long userId, MessageSendFileRequestDto dto) {
        ...
        BinaryMessage newMessage = messageService.saveFile(
                chatId,
                userId,
                Mapper.map(dto, BinaryMessage.class)
        );
    ...
    private Action getAction(Message message) {
        ...
        MessageSendFileResponseDto newMessageDto = MessageSendFileResponseDto.builder()
                .chat(chatDto)
                .user(userDto)
                .message(messageDto)
                .build();
